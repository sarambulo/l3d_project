...
def voxelize_mesh_trimesh(verts, faces, resolution=64):
    ...
    min_xyz = min_xyz - range_xyz * padding
    max_xyz = max_xyz + range_xyz * padding
    

    x = np.linspace(min_xyz[0], max_xyz[0], resolution)
    y = np.linspace(min_xyz[1], max_xyz[1], resolution)
    z = np.linspace(min_xyz[2], max_xyz[2], resolution)
    
    grid_x, grid_y, grid_z = np.meshgrid(x, y, z, indexing='ij')
    grid_points = np.stack([grid_x.ravel(), grid_y.ravel(), grid_z.ravel()], axis=1)
    
    # Check which points are inside the mesh
    # trimesh.contains() is very stable and fast
    inside = mesh.contains(grid_points)
    inside = inside.reshape(resolution, resolution, resolution)
    
    # Convert to torch tensor
    voxels = torch.from_numpy(inside.astype(np.float32))
    min_xyz_torch = torch.from_numpy(min_xyz).float()
    max_xyz_torch = torch.from_numpy(max_xyz).float()
    
    return voxels, (min_xyz_torch, max_xyz_torch)
...
def primitive_coverage_loss(
    ...
    for b in range(B):
        verts_b = gt_verts[b]  
        faces_b = gt_faces[b] 
        gt_voxels, (min_xyz, max_xyz) = voxelize_mesh_trimesh(
            verts_b, faces_b, resolution=resolution
        )
        
        
        gt_voxels = gt_voxels.to(device)
        min_xyz = min_xyz.to(device)
        max_xyz = max_xyz.to(device)
        
       
        x = torch.linspace(min_xyz[0].item(), max_xyz[0].item(), resolution, device=device)
        y = torch.linspace(min_xyz[1].item(), max_xyz[1].item(), resolution, device=device)
        z = torch.linspace(min_xyz[2].item(), max_xyz[2].item(), resolution, device=device)
        ...
        prim_voxels = (sdf < 0).float().reshape(resolution, resolution, resolution)
        
        # Compute MSE loss between GT and primitive voxels
        loss = F.mse_loss(prim_voxels, gt_voxels)
